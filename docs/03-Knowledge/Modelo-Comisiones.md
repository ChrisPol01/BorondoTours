...
```
Día 1: Cliente paga $200.000 → llega a cuenta OnePay.la de BorondoTours
...
### Comisiones de Agentes de Ventas (H-19)

- El % de comisión de cada agente es **mensual y configurable** por el GERENTE
- Cada cambio de % se registra en `AgentCommissionHistory` (historial inmutable)
- El % vigente se usa para calcular la comisión de cada booking
- El agente ve su historial de comisiones en el ERP pero NO puede editarlo
- La comisión se calcula sobre el **subtotal antes de IVA** del booking

### Gamificación de Ventas para Agentes (H-20)

- **Ka-ching Sound:** Al confirmar un pago, el agente escucha un sonido de caja registradora
- **Leaderboard:** Ranking en tiempo real de ventas del mes actual (visible en el sidebar del ERP)
- **Metas Mensuales:** El GERENTE define meta de ventas en COP por agente
  - Barra de progreso en el dashboard del agente
  - Al alcanzar la meta: badge animado + notificación al equipo
- **Streak:** Contador de días consecutivos con al menos 1 venta
  - Se reinicia si un día no hay ventas
  - El streak más largo se muestra en el perfil del agente
...
| OnePay.la hace chargeback | BorondoTours asume el chargeback. Si el operador ya fue pagado, se descuenta del siguiente payout (ver `OperatorChargebacks`) |
| Reembolso bancario por retracto | Payout se revierte. Si ya fue pagado, se descuenta del siguiente payout |
