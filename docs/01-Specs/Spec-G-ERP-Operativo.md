...
- [ ] Tours propios de BorondoTours (Fase 2+): Usan un operador interno especial (`operator_id = BORONDO_INTERNAL_ID`) con `commission_rate = 0%`. Se gestionan directo desde el ERP y no requieren aprobación de ningún tipo.
...
- [ ] Conteo de cupos atómico: La actualización de `cupos_disponibles` / `bookings_count` debe realizarse mediante transacciones bloqueantes (ej. `SELECT FOR UPDATE` o `UPDATE ... RETURNING`) para evitar race conditions y sobreventas.
...
**Criterios de aceptación:**
- [ ] Liquidaciones por ciclo de corte (mensual o quincenal, configurable por contrato), no por reserva individual.
- [ ] Al cerrar el ciclo, el sistema consolida todos los bookings de tours ejecutados en ese período.
- [ ] El cálculo de `operator_amount` abandona la fórmula lineal y aplica los 4 Casos Matemáticos descritos en `Modelo-Comisiones` (B2C Full, B2C Reserva, B2B Agencia Full, B2B Agencia Split).
- [ ] Se crea un registro en `OperatorPayouts` agrupando todas las transacciones del ciclo en estado `PENDING`.
- [ ] El coordinador marca el pago como `PAID` cuando transfiere al operador, adjuntando soporte.
- [ ] El operador ve en su portal el historial de liquidaciones (pendientes y pagadas) con su PDF de desglose.
- [ ] Reporte exportable: liquidaciones por operador por período (CSV/PDF).
