...
- [ ] Widget de Calidad del Servicio (H-39): Muestra el Rating promedio, Tasa de No Show y Tasa de Quejas/Reembolsos comparada con el promedio de la plataforma.
...
- [ ] Formulario de creación con wizard de 4 pasos:
  ...
  3. **Pricing y reglas**: precio base (SLA: mínimo $5,000 COP, máximo $30,000,000 COP), regla de pax (por edad o estatura), rangos de precio por categoría, **add-ons** opcionales (con nombre, precio, tiempo/duración de uso, cantidad máxima permitida por pax, y stock disponible por instancia).
  4. **Revisión**: preview de cómo se verá el tour en el B2C + botón "Enviar a revisión"
- [ ] Al enviar: el tour queda en estado `PENDING_CREATE_REVIEW` (H-10)
- [ ] BorondoTours (SUPER_ADMIN o GERENTE) revisa y aprueba → `PUBLISHED` o rechaza con comentarios → `REJECTED`
- [ ] SLA de Revisión (H-49): 24h hábiles. Si excede el SLA, alerta visual en el portal del SUPER_ADMIN y notificación al creador del tour informando la demora.
...
- [ ] Al enviar: se crea una `TourEditRequest` con estado `PENDING_EDIT_REVIEW` (H-10)
- [ ] BorondoTours revisa el diff (qué cambió) y aprueba/rechaza
- [ ] Si se aprueba: los cambios se aplican al tour publicado inmediatamente. El operador recibe Notificación Push in-app y Email con el resumen de los cambios aplicados (H-38).
...
- [ ] Cancelación por Baja Demanda (H-62): Si una instancia tiene < 30% de ocupación y faltan > 48h, el operador puede iniciar flujo `LOW_DEMAND`.
  - El sistema inicia la devolución automática a los clientes B2C afectados, reintegrando con un 105% en Borondo Coins o reembolso a original.
  - Genera penalidad automática en la métrica del operador.
  - Activa la generación de ticket de PQRS asociado a la cancelación.
...
- [ ] Al abrir el link, la agencia externa ve una página pública (sin login) con:
  ...
  - Banner dinámico con "Vence en Xh Ym" + *Countdown visual en rojo si faltan menos de 24h* (H-37).
...
- [ ] Si BorondoTours suspende al operador (H-35): las instancias con reservas confirmadas seguirán ejecutándose y serán visibles en su radar, pero **se bloquean todas las nuevas ventas** en cualquier canal y sus instancias futuras sin ventas se ocultan.
...
- [ ] Identificador y deducción explícita de `OperatorChargebacks` (si hay disputas o reembolsos por soporte técnico descontados del payout, H-36).
...
Tours (extensión)
  - status: enum (DRAFT, PENDING_CREATE_REVIEW, PUBLISHED, REJECTED, SUSPENDED)
...
TourEditRequests
  ...
  - status: enum (PENDING_EDIT_REVIEW, APPROVED, REJECTED)
  - review_notes: text | null
  - reviewed_by: uuid FK | null
  - created_at: timestamp
  - resolved_at: timestamp | null

OperatorChargebacks (H-36)
  - id: uuid
  - operator_id: uuid FK
  - booking_id: uuid FK | null
  - reason: string
  - amount: decimal
  - deducted_from_payout_id: uuid FK | null
  - created_at: timestamp
...
Users (extensiones)
  ...
  # Nota (H-11): Se eliminó operator_role explícitamente ya que el role principal del usuario es la única fuente de verdad (ej: OPERATOR_ADMIN).
