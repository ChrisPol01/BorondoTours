...
- [ ] 5 niveles basados en **gasto histórico acumulado en COP**:

| Nivel | Nombre | Umbral COP acumulado | % Coins por compra | Beneficios extra |
|---|---|---|---|---|
| 1 | Explorador | $0+ | 0.5% | Acceso básico |
| 2 | Viajero | $500.000+ | 1% | Soporte prioritario por email |
| 3 | Aventurero | $1.500.000+ | 1.5% | 3% descuento en add-ons |
| 4 | Conquistador | $3.000.000+ | 2% | 5% dcto add-ons, sin anuncios en checkout |
| 5 | Embajador | $6.000.000+ | 3% | Sin anuncios en todo el flujo, acceso anticipado a nuevos tours |
...
- [ ] En el perfil del usuario se muestra: nivel actual, barra de progreso al siguiente nivel, COP faltantes
...
**Contexto:** Los Coins se otorgan al momento en que el pago es CONFIRMED por el webhook de Bold.

**Criterios de aceptación:**
- [ ] Cálculo: `Coins = floor(precio_tour_base × % nivel / 100)` (en pesos COP, 1 Coin = $1 COP)
...
- [ ] Transacción registrada en `CoinTransactions` con tipo `EARNED`
...
- [ ] Al expirar: transacción `EXPIRED` en `CoinTransactions`, saldo se reduce
...
- [ ] Filtros en la tabla: por tipo (EARNED, USED, EXPIRED, REFUNDED), por fecha
...
- [ ] `EARNED` — Coins ganados por compra confirmada
- [ ] `USED` — Coins aplicados como descuento en checkout
- [ ] `EXPIRED` — Coins restringidos vencidos
- [ ] `REFUNDED` — Coins devueltos si una compra es reembolsada (booking cancelado)
- [ ] `MANUAL` — Ajuste manual por SUPER_ADMIN o COORD (con nota obligatoria)
- [ ] `COMPENSACION` — Coins restringidos otorgados por cancelación del operador
...
LoyaltyLevels (tabla de configuración — editable desde ERP)
  ...
  - nombre: string
  - umbral_cop: decimal
  - pct_coins: decimal (0.5, 1, 1.5, 2, 3)
  - beneficios: jsonb

UserLoyalty
  - user_id: uuid FK (1:1 con Users)
  - current_level: integer (1–5)
  - total_spent_cop: decimal      ← acumulado histórico
  - free_coins_balance: decimal
  - updated_at: timestamp

CoinTransactions
  - id: uuid
  - user_id: uuid FK
  - type: enum (EARNED, USED, EXPIRED, REFUNDED, MANUAL, COMPENSACION)
  - amount: decimal               ← positivo = ganó, negativo = usó/expiró
  - is_restricted: boolean
  - operator_id: uuid FK | null   ← solo si is_restricted = true
  - booking_id: uuid FK | null
  - expires_at: timestamp | null  ← solo si is_restricted = true
  - note: string | null           ← obligatorio si type = MANUAL
