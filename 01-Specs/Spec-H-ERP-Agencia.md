...
| 2 | **Reserva** | El cliente pagó el % de reserva (Split Fare parcial o reserva completa) | Automático vía webhook Bold |
| 3 | **Pagado** | Pago total confirmado por Bold webhook | Automático vía webhook Bold |
...
- [ ] Al hacer clic en la tarjeta: panel lateral (drawer) con:
  ...
  - Botón "Reenviar link" (genera un nuevo link Bold o reenvía el existente)
...
- [ ] Al guardar: la cotización entra automáticamente en columna **Cotizado** del Kanban

### RF-H07 — Generar link de pago Bold (Link Mágico)
**Contexto:** El agente genera un link Bold pre-configurado con todos los parámetros de la cotización. El cliente solo abre el link y paga, sin necesidad de navegar por el portal B2C.

**Criterios de aceptación:**
- [ ] Botón "Generar Link de Pago" en la tarjeta de cotización
- [ ] El sistema llama a la API de Bold para crear un payment link con:
...
- [ ] Si el link Bold expira sin pago: se puede regenerar (nuevo link, nuevo `expiration`)
...
- [ ] El mensaje de WhatsApp incluye: nombre del cliente, nombre del tour, fecha, monto y el link Bold
...
### RF-H09 — Webhook Bold → actualización automática del Kanban
**Criterios de aceptación:**
- [ ] El webhook `/api/v1/webhooks/bold` distingue entre pagos B2C y pagos de cotizaciones por el prefijo del `reference`:
...
POST   /api/v1/erp/quotes/:id/generate-link   → Generar link Bold (AGENT, GERENTE)
...
Quotes
  ...
  - iva_amount: decimal
  - total: decimal
  - source: enum (WEB_B2C, AGENT_MANUAL, REFERIDO)
  - status: enum (COTIZADO, RESERVA, PAGADO, EJECUCION, COMPLETADO, PERDIDO, EXPIRED)
  - lost_reason: string | null
  - bold_link_url: string | null
  - bold_link_expires_at: timestamp | null
  - bold_link_opened_at: timestamp | null   ← tracking de apertura
...
|---|---|---|
| Webhook Bold (pagos de cotizaciones) | Spec-C RF-C WebhookBold | Distinguir por prefijo `QUOTE-` vs `BOOK-` en `reference` |
